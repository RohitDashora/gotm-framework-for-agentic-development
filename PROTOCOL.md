...
4. Identify the **active unit** (top non-`done` row whose blockers are clear).
5. Act on that unit, or — if a higher-priority concern surfaces — surface it.
6. After acting, write back: update `LEDGER.md`, append to `DECISIONS.md` if a decision was made, update `QUESTIONS.md` if a question opened or closed.

These six steps are not negotiable. The discipline depends on the project's state being the state of these files.
...
- An audit needs an independent context — the agent that did the work cannot audit the same work (Rule 4).
- Multiple atomic units can run in parallel without shared state.

The dispatch prompt carries:

- A pointer to `PROTOCOL.md` (this file) — so the worker reads the discipline.
...
For projects forking this repo: replace the line above with your project's mission. One sentence; what does done look like.
