...
3. **Assign each ready unit a Tier, then dispatch a worker per unit (fan out); proof-stamp DISPATCHED.** You are the **allocator** — fixed at the user's strongest setting (generally SOTA + xhigh), you never tier *yourself*; you tier each **worker**. As you form the ready set and dispatch, assign every ready unit a **`Tier`** by the rubric, guided-not-ruled (first match wins): **R1 → frontier** — irreversible blast radius **OR** diagnosis/design **OR** deep-reasoning + high-novelty; **R2 → standard** — critical-path hub **OR** authoring/synthesis **OR** multi-hop **OR** hard-to-audit; **R3 → economy** — mechanical, low-reasoning, reversible, easily audited; **unsure → standard (default)**. The unit's `Kind` largely **predicts** its Tier (eval/deploy-infra/diagnosis → frontier; ui/mechanical → economy), so this piggybacks on the dispatch reasoning you already do — no separate router, no extra pass. Then for each ready unit, dispatch a fresh worker with that unit's bounded payload only — its inputs and spec, nothing more (`worker-dispatch.md`). **At spawn, resolve the unit's `Tier → (model, effort)` and set the worker's resources accordingly** — the worker never reads its own tier (where the runtime can't set per-worker effort, degrade to model-only tiering). Dispatch independent ready units **together, in parallel**, the way a Spark stage runs its tasks at once. Bound the width with a **concurrency cap** (backpressure): dispatch K at a time, refill as they finish. Ready units do not queue behind each other for no reason. **Stamp a unit `in_progress`/DISPATCHED only *after* the Agent launch returns an id** — the id is proof the worker actually launched. Never write the DISPATCHED stamp on intent alone; the ledger records verified fact, not a plan to dispatch. On turn-end, rescan that no `in_progress`/DISPATCHED row lacks a real launched id.
...
6. **On worker failure, retry-with-escalation (kill-and-respawn one tier up).** On any of **three triggers, cheapest first** — (1) a worker's **`ESCALATE: <reason>`** self-signal (out of its depth; a hint, not trusted — the audit stays the backstop); (2) a **watchdog** hit — a worker that hangs, loops, or blows a liveness timer; **kill it if it is still running**, without waiting for a bad return; (3) an **audit-FAIL** (the reliable backstop; the worker is already gone) — re-dispatch the *same* unit to a **fresh** worker on the same on-disk inputs, **one tier up** on the escalation ladder (**economy → standard → frontier**). This is safe by construction: a unit is a function of its bounded inputs, and those inputs live on disk — re-running it is a **lineage recompute**, not a salvage of partial state. Killing is clean because **you are the single writer** (a killed worker's partial output never lands in the ledger, so the store is untouched and the fresh worker re-runs from clean inputs) and because **irreversible units are frontier from the start** (rubric R1) — so the only workers ever killed-and-respawned are reversible/scratch ones; you never kill a worker mid-side-effect (mid-`deploy` / mid-write). **Cap** at frontier (**max 2 bumps / unit**); a **frontier failure is a hard stop → human** via the ratification ladder, **never a loop**. **Record** every escalation (start tier, final tier, trigger, reason) — a mis-tiering signal for rubric tuning and a learning-pool fact.
