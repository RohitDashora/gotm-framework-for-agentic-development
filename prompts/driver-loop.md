...
6. **Triage upward signals inline.** If a worker returned a typed signal (`split` / `discovery` / `blocker`), read it inline and triage: **mint** a new unit, **reshape** its scope, **merge** it into existing work, **absorb** it (known concern), **route to human** (QUESTIONS), or **decline with durable reason** (one-liner in recovery log, preventing re-raise). The worker suggests; you decide — advisory only. Signals close here; no standing inbox.
...
11. **Repeat until the DAG drains.** Recompute the ready set; dispatch gate → dispatch → collect → triage signals → audit → retry → milestone-check → checkpoint. When the ready set empties and nothing is blocked, the project is done.
