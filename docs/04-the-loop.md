...
All three are the *same loop*. The command and the script do not change what the scheduler does; they only harden how reliably it is followed. A reader adopting GOTM with nothing but a prompt is running the real thing; the tooling is convenience and rigor, not a different mechanism.
